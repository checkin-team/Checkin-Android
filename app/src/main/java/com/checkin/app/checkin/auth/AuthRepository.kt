...
class AuthRepository private constructor(context: Context) : BaseRepository() {
    ...
    fun login(credentials: ObjectNode): LiveData<Resource<AuthResultModel>> {
        return object : NetworkBoundResource<AuthResultModel, AuthResultModel>() {
            override fun shouldUseLocalDb(): Boolean = false

            override fun createCall(): LiveData<ApiResponse<AuthResultModel>> = RetrofitLiveData(mWebService.postLogin(credentials))

            override fun saveCallResult(data: AuthResultModel?) {}
        }.asLiveData
    }

    fun register(credentials: ObjectNode): LiveData<Resource<AuthResultModel>> {
        return object : NetworkBoundResource<AuthResultModel, AuthResultModel>() {
            override fun shouldUseLocalDb(): Boolean = false

            override fun createCall(): LiveData<ApiResponse<AuthResultModel>> = RetrofitLiveData(mWebService.postRegister(credentials))
