...
class QSRSessionDetailActivity : BaseActivity() {
            ...
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                when (message.type) {
                    ...
                    MESSAGE_TYPE.USER_SCHEDULED_QSR_CHECKOUT, MESSAGE_TYPE.USER_SCHEDULED_QSR_CANCELLED -> {
                        Utils.toast(this@QSRSessionDetailActivity, "Session ended!")
                        finish()
                    }
                    MESSAGE_TYPE.USER_SCHEDULED_QSR_DONE -> {
                        startActivity(QSRFoodReadyActivity.withSessionIntent(this@QSRSessionDetailActivity, session.pk))
                        finish()
                    }
                }
            }
    ...
    companion object {
        ...
        fun withSessionIntent(context: Context, sessionId: Long) = Intent(context, QSRSessionDetailActivity::class.java).apply {
