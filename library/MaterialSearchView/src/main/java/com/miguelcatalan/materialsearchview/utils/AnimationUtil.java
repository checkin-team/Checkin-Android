...
public class AnimationUtil {
    ...
    public static void reveal(final View view, final AnimationListener listener, boolean startFromRight) {
        int cx = 0;
        if (startFromRight)
            cx = view.getWidth() - (int) TypedValue.applyDimension(
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static void hide(final View view, final AnimationListener listener, boolean startFromRight) {
        int cx = 0;
        if (startFromRight)
            cx = view.getWidth() - (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 24, view.getResources().getDisplayMetrics());
